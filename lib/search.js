...
async function search (q) {
  const { data } = await octokit.search.issues(getSearchParams({ q }))
  ...
  if (data.total_count > data.items.length) {
    ...
    const randomPage = Math.floor(Math.random() * Math.floor(lastPageAllowed)) + 1

    const issues = await octokit.search.issues(getSearchParams({ q: q, page: randomPage }))
    return transform(issues.data)
  } else {
